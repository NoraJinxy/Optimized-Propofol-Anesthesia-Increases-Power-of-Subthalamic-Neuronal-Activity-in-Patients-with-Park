function violins = violinplot(data, cats,ColorDesign,varargin)
...
%     order=cellstr(order);
    ...
    elseif hascategories && numel(data) == numel(cats)
        if isempty(grouporder)
            cats = categorical(cats,order,'Ordinal',true);
        else
        ...
        for n=1:length(catnames)
            if mod(n,2)==0
                thisColor=ColorDesign(1,:);
            else
                thisColor=ColorDesign(2,:);
            end
            thisCat = catnames{n};
            thisData = data(cats == thisCat);
            violins(n) = Violin(thisData, n, varargin{:},'violincolor',thisColor);
            catnames1{n}=textscan(thisCat, '%s ');
        end
        set(gca, 'xtick', 1:length(catnames), 'xticklabels', catnames);
%         set(gca, 'xtick', 1:length(catnames), 'xticklabels', catnames1{1,n}{1,1}(:));
